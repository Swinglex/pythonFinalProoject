...
class TestBueWillConquerJarrett:
    @pytest.fixture
    def client(self):
        with app.test_client() as client:
            yield client

    def test_approute(self, client):
        response_index = client.get('/')
        response_room = client.get('/room')
        response_supply = client.get('/supply')

        assert response_index.status_code == 200
        assert response_room.status_code == 200
        assert response_supply.status_code == 200

    def test_room(self, client):
